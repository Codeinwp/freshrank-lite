<?php
/**
 * Plugin Name: FreshRank AI
 * Plugin URI:   https://wordpress.org/plugins/freshrank-ai/
...
if ( defined( 'FRESHRANK_AI_ACTIVE_PLUGIN' ) && FRESHRANK_AI_ACTIVE_PLUGIN !== $current_plugin_basename ) {
	...
	add_action(
		...
		function () use ( $current_plugin_basename ) {
			...
			$current_label   = ( strpos( $current_plugin_basename, 'freshrank-ai-pro' ) !== false )
			? __( 'FreshRank AI Pro', 'freshrank-ai' )
			: __( 'FreshRank AI Lite', 'freshrank-ai' );
			$active_label    = ( strpos( $active_basename, 'freshrank-ai-pro' ) !== false )
			? __( 'FreshRank AI Pro', 'freshrank-ai' )
			: __( 'FreshRank AI Lite', 'freshrank-ai' );
...
define( 'FRESHRANK_VERSION', '0.0.1' );
...
// Free version (freshrank-ai): Analysis only, limited features
// Pro version (freshrank-ai-pro): Full features including content generation
if ( ! defined( 'FRESHRANK_FREE_VERSION' ) ) {
...
if ( ! function_exists( 'freshrank_is_free_version' ) ) {
...
if ( ! function_exists( 'freshrank_feature_available' ) ) {
...
if ( ! function_exists( 'freshrank_debug_log' ) ) {
...
if ( ! function_exists( 'freshrank_whitelabel_enabled' ) ) {
	function freshrank_whitelabel_enabled() {
		if ( freshrank_is_free_version() ) {
			return false;
		}
		return (bool) get_option( 'freshrank_whitelabel_enabled', 0 );
	}
}

// Get white-labeled plugin name
if ( ! function_exists( 'freshrank_get_plugin_name' ) ) {
...
if ( ! function_exists( 'freshrank_get_logo_url' ) ) {
...
if ( ! function_exists( 'freshrank_get_primary_color' ) ) {
...
if ( ! function_exists( 'freshrank_get_support_email' ) ) {
...
if ( ! function_exists( 'freshrank_get_docs_url' ) ) {
...
if ( ! function_exists( 'freshrank_hide_branding' ) ) {
	function freshrank_hide_branding() {
		if ( freshrank_whitelabel_enabled() ) {
			return get_option( 'freshrank_whitelabel_hide_branding', 0 );
		}
		return false;
	}
}
...
if ( ! function_exists( 'freshrank_custom_instructions_enabled' ) ) {
...
if ( ! function_exists( 'freshrank_get_custom_analysis_prompt' ) ) {
...
if ( ! function_exists( 'freshrank_get_custom_rewrite_prompt' ) ) {
...
if ( ! function_exists( 'freshrank_get_gsc_api' ) ) {
